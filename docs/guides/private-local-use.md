...
Run `node scripts/nectar-dispatch-bridge.js --check-env` before starting the bridge when you want a non-mutating readiness check; it now also reports existing `inbox_record_count`, `pending_inbox_count`, and a bounded `pending_inbox_names` preview so operators can see pending local handoffs without binding a port.
