...
Run `node scripts/nectar-dispatch-bridge.js --check-env` before starting the bridge when you want a non-mutating readiness check; it now also reports existing `inbox_record_count`, `pending_inbox_count`, `pending_inbox_attention_reason`, a bounded `pending_inbox_names` preview, and `pending_inbox_next_name` / `pending_inbox_next_path` so operators can find the next local handoff without binding a port.
