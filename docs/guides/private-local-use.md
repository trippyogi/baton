...
Run `node scripts/nectar-dispatch-bridge.js --check-env` before starting the bridge when you want a non-mutating readiness check; it now also reports existing `inbox_record_count`, `pending_inbox_count`, `pending_inbox_attention_reason`, `pending_inbox_attention_level` (`none`, `low`, `medium`, or `high`), `pending_inbox_preview_count`, a bounded `pending_inbox_names` preview, `pending_inbox_next_name` / `pending_inbox_next_path`, command hints (`check_env_command`, `start_command`, `next_inbox_command`), `pending_inbox_review_command` when local work is waiting, `verification_scope`, and an `operator_next_check` that points at the next pending local handoff when one exists, so operators can find the next prompt without binding a port. Run `node scripts/nectar-dispatch-bridge.js --next-inbox` to print the oldest pending local handoff record and its generated prompt as JSON without starting the bridge; the response includes explicit `pending_inbox_next_received_at`, `pending_inbox_next_age_seconds`, `pending_inbox_next_age_bucket`, `pending_inbox_review_command`, `inbox_record_sha256`, `inbox_record_hash_algorithm`, `prompt_present`, `prompt_length`, `prompt_sha256`, and command hints so a local operator can verify what will be handed to Nectar before copying it.
