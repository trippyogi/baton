...
Run `node scripts/nectar-dispatch-bridge.js --check-env` before starting the bridge when you want a non-mutating readiness check; it now also reports existing `inbox_record_count`, `pending_inbox_count`, `pending_inbox_attention_reason`, `pending_inbox_attention_level` (`none`, `low`, `medium`, or `high`), `pending_inbox_preview_count`, a bounded `pending_inbox_names` preview, `pending_inbox_next_name` / `pending_inbox_next_path`, command hints (`check_env_command`, `start_command`, `next_inbox_command`, `next_inbox_prompt_command`, and `next_inbox_path_command`), `pending_inbox_review_command` / `pending_inbox_prompt_command` / `pending_inbox_path_command` / `pending_inbox_next_prompt_command` when local work is waiting, `verification_scope`, and an `operator_next_check` that points at the next pending local handoff when one exists, so operators can find the next prompt without binding a port. Run `node scripts/nectar-dispatch-bridge.js --next-inbox` to print the oldest pending local handoff record and its generated prompt as JSON without starting the bridge; the response includes explicit `pending_inbox_next_received_at`, `pending_inbox_next_age_seconds`, `pending_inbox_next_age_bucket`, `pending_inbox_review_command`, `pending_inbox_prompt_command`, `pending_inbox_path_command`, `inbox_record_sha256`, `inbox_record_hash_algorithm`, `prompt_present`, `prompt_length`, `prompt_sha256`, and command hints so a local operator can verify what will be handed to Nectar before copying it. Use `node scripts/nectar-dispatch-bridge.js --next-inbox --prompt-only` only for a private local handoff where you intentionally want just the prompt text; use `--path-only` when automation only needs the ignored local inbox path, or `--summary-only` when health/ACK automation should show safe queue metadata without printing the private envelope or prompt.

The bridge accepts `baton.dispatch.v1`, writes an ignored local inbox record with `schema_version: baton.nectar_bridge.inbox_record.v1` under `local/nectar-dispatch-inbox/`, and returns an ACK. Inbox writes use create-only semantics so an accidental duplicate dispatch cannot overwrite an existing local prompt; duplicate submissions return HTTP 409 with `rejection_code: duplicate_dispatch` and the existing `inbox_record_name`. Accepted and rejected dispatch responses include stable `schema_version: baton.nectar_bridge.dispatch_result.v1`, `bridge_version`, `bridge_request_id`, and `generated_at`; accepted ACKs echo `dispatch_id`, `run_id`, `task_id`, `touch_id`, `inbox_record_name`, and `prompt_sha256` plus `received_count`/`inbox_record_count` for traceable local logs, while rejections include `rejection_code` and `error_count` for client-side logging. It requires `Content-Type: application/json`, rejects malformed JSON, malformed callback URLs, callback URLs with embedded credentials, and oversized request bodies before writing inbox records, including a fast `Content-Length` preflight when clients provide it; set `NECTAR_BRIDGE_MAX_BODY_BYTES` to a positive integer only if a larger local envelope limit is explicitly needed; invalid values are ignored in favor of the safe default. Its local `/health` endpoint supports GET and HEAD probes; GET includes `health_schema_version`, `bridge_version`, `bridge_instance_id`, `generated_at`, `bind_host`, `dispatch_path`, `accepted_content_type`, `accepted_methods`, `dispatch_url`, command hints (`check_env_command`, `start_command`, `next_inbox_command`, `next_inbox_summary_command`, `pending_inbox_review_command`, and `pending_inbox_next_prompt_command` when local work is waiting), `token_required`, `bridge_status` (`idle`, `needs_client_fix`, `ready_to_process`, or `blocked_inbox_unwritable`), `verification_scope`, `latest_activity_at`, `latest_activity_source`, `started_at`, `uptime_seconds`, `received_count`, `rejected_count`, `inbox_record_count`, `pending_inbox_count` (pending records only, not completed inbox history), `inbox_processing_status_counts`, `pending_inbox_preview_limit`, `pending_inbox_preview_count`, `first_pending_inbox_name`, `first_pending_inbox_path`, `pending_inbox_next_name`, `pending_inbox_next_path`, oldest/newest pending inbox names/paths/timestamps/age seconds, `inbox_dir`, `inbox_record_schema_version`, `inbox_writable`, `last_received_at`, `last_received_request_id`, `last_received_dispatch_id`, `last_received_run_id`, `last_received_task_id`, `last_received_touch_id`, `last_inbox_path`, `last_inbox_name`, `last_prompt_sha256`, `last_prompt_hash_algorithm`, `last_rejected_at`, `last_rejection_request_id`, `last_rejection_status`, `last_rejection_reason`, `last_rejection_code`, `last_rejection_errors`, `last_rejection_error_count`, `max_body_bytes`, and `operator_next_check` for quick smoke/debug checks. Accepted ACKs also include `inbox_processing_status_counts` and `next_inbox_command` so a local operator can see pending/completed/unknown inbox state and fetch the next prompt without opening every record. Accepted ACKs and rejection responses also include `operator_next_check` so local automation can surface the next safe handoff or client-fix step without reinterpreting error codes. It does not execute external actions by itself.
...
2. If `pending_inbox_count` is greater than `0`, open `pending_inbox_next_path` (alias of `first_pending_inbox_path`) under this repo, or run `node scripts/nectar-dispatch-bridge.js --next-inbox` to print the oldest pending record and prompt as JSON.
...
The local Nectar bridge health and accepted ACK payloads include `pending_inbox_paths`, `pending_inbox_preview_limit`, `pending_inbox_preview_count`, plus the first/oldest pending inbox name/path (`first_pending_inbox_*` and `pending_inbox_oldest_*`, ordered by inbox record `received_at` rather than filename). Health also exposes `pending_inbox_oldest_age_seconds`, `pending_inbox_oldest_age_bucket` (`none`, `fresh`, `warm`, `stale`, `old`), `pending_inbox_attention_reason` (`none`, `pending_inbox_waiting`, or `pending_inbox_stale`), `pending_inbox_attention_level` (`none`, `low`, `medium`, or `high`), and `pending_inbox_newest_age_seconds` so operators can spot stale local handoffs quickly. Use these ignored local paths to hand the oldest pending dispatch prompt to Nectar/OpenClaw without guessing filenames; if `pending_inbox_has_overflow` is true or `pending_inbox_overflow_count` is non-zero, process or archive older local inbox records before relying only on the preview list.
