...
- **Local Nectar dispatch bridge**
  ...
  - `--check-env` performs a non-mutating local readiness check and reports existing `inbox_record_count`, `pending_inbox_count`, `pending_inbox_attention_reason`, `pending_inbox_preview_count`, a bounded pending inbox preview, and explicit `next_inbox_command` / `next_inbox_prompt_command` hints before the bridge binds a port.
  - `/health` supports GET and HEAD probes and exposes `bridge_version`, `bridge_instance_id`, `generated_at`, `bind_host`, `dispatch_path`, `dispatch_url`, `started_at`, `uptime_seconds`, `received_count`, `rejected_count`, `inbox_record_count`, `inbox_dir`, `inbox_record_schema_version`, `inbox_writable`, `last_received_at`, `last_received_request_id`, `last_received_dispatch_id`, `last_received_run_id`, `last_received_task_id`, `last_received_touch_id`, `last_inbox_path`, `last_prompt_sha256`, `local_handoff_status`, `pending_inbox_preview_limit`, `pending_inbox_preview_count`, `pending_inbox_paths`, `pending_inbox_review_command` and `next_inbox_summary_command` when local work is waiting, `pending_inbox_attention_reason`, oldest/newest pending inbox timestamps and ages, `last_rejected_at`, `last_rejection_request_id`, `last_rejection_reason`, `last_rejection_errors`, `last_rejection_error_count`, and `max_body_bytes` for smoke/debug checks.
