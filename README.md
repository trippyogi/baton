...
- **Local Nectar dispatch bridge**
  ...
  - `--check-env` performs a non-mutating local readiness check and reports existing `inbox_record_count`, `pending_inbox_count`, `pending_inbox_attention_reason`, `pending_inbox_preview_count`, and a bounded pending inbox preview before the bridge binds a port.
