...
- **Local Nectar dispatch bridge**
  ...
  - `/health` supports GET and HEAD probes and exposes `started_at`, `uptime_seconds`, `received_count`, `inbox_record_count`, `inbox_writable`, `last_received_at`, and `max_body_bytes` for smoke/debug checks.
