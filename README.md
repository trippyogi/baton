...
- **Local Nectar dispatch bridge**
  ...
  - The bridge ACKs and writes ignored inbox records under `local/nectar-dispatch-inbox/`; accepted/rejected dispatch responses include `schema_version: baton.nectar_bridge.dispatch_result.v1` and `generated_at`, and accepted ACKs echo dispatch/run/task/touch ids for local traceability; it does not execute external actions by itself.
