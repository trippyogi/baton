...
- **Local Nectar dispatch bridge**
  ...
  - The bridge ACKs and writes ignored inbox records under `local/nectar-dispatch-inbox/`; accepted/rejected dispatch responses include `schema_version: baton.nectar_bridge.dispatch_result.v1`, `bridge_version`, and `generated_at`, and accepted ACKs echo dispatch/run/task/touch ids, `inbox_record_schema_version`, and `inbox_processing_status: pending_local_operator` for local traceability; it does not execute external actions by itself.
  ...
  - `/health` supports GET and HEAD probes and exposes `bridge_version`, `generated_at`, `bind_host`, `dispatch_path`, `dispatch_url`, `started_at`, `uptime_seconds`, `received_count`, `rejected_count`, `inbox_record_count`, `inbox_dir`, `inbox_record_schema_version`, `inbox_writable`, `last_received_at`, `last_received_dispatch_id`, `last_received_run_id`, `last_received_task_id`, `last_received_touch_id`, `last_inbox_path`, `last_rejected_at`, `last_rejection_reason`, `last_rejection_errors`, `last_rejection_error_count`, and `max_body_bytes` for smoke/debug checks.
