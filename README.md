...
- **Local Nectar dispatch bridge**
  ...
  - `/health` supports GET and HEAD probes and exposes `bridge_version`, `bridge_instance_id`, `generated_at`, `bind_host`, `dispatch_path`, `dispatch_url`, `started_at`, `uptime_seconds`, `received_count`, `rejected_count`, `inbox_record_count`, `inbox_dir`, `inbox_record_schema_version`, `inbox_writable`, `last_received_at`, `last_received_request_id`, `last_received_dispatch_id`, `last_received_run_id`, `last_received_task_id`, `last_received_touch_id`, `last_inbox_path`, `pending_inbox_preview_limit`, `pending_inbox_paths`, `last_rejected_at`, `last_rejection_request_id`, `last_rejection_reason`, `last_rejection_errors`, `last_rejection_error_count`, and `max_body_bytes` for smoke/debug checks.
