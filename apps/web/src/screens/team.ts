...
<div class="card card-kpi" style="border-radius:var(--radius-card)">
  ...
  <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px">
    ...
    <div>
      <div style="font-size:20px;font-weight:700;color:var(--text-primary)">${escapeHtml(a.name)}</div>
      <div style="font-size:12px;color:var(--text-secondary);margin-top:2px">${escapeHtml(a.role)}</div>
    </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
    ${metaField('Discord ID', `<@${escapeHtml(a.id)}>`, 'font-family:var(--font-instrument);font-size:12px')}
...
function metaField(label, value, valueStyle = '') {
  return `<div>
    ...
    <div style="font-size:13px;color:var(--text-primary);${valueStyle}">${escapeHtml(value)}</div>
  </div>`;
}
