...
function checkBridgeEnvironment(config = bridgeConfigFromEnv()) {
  ...
  return {
    ...
    pending_inbox_next_received_at: oldestPendingInboxReceivedAt,
    pending_inbox_next_age_seconds: pendingInboxOldestAgeSeconds,
    pending_inbox_next_age_minutes: minutesFromSeconds(pendingInboxOldestAgeSeconds),
    pending_inbox_next_age_bucket: pendingInboxOldestAgeBucket,
    ...
    local_handoff_status: pendingInboxNames.length > 0 ? 'pending_local_operator' : 'idle',
...
function readNextInboxRecord(config = bridgeConfigFromEnv()) {
  ...
  return {
    ...
    pending_inbox_count: pendingInboxNames.length,
    pending_inbox_preview_limit: PENDING_INBOX_PREVIEW_LIMIT,
    pending_inbox_preview_count: pendingInboxPreviewNames.length,
    pending_inbox_names: pendingInboxPreviewNames,
    pending_inbox_has_overflow: pendingInboxNames.length > PENDING_INBOX_PREVIEW_LIMIT,
    pending_inbox_overflow_count: Math.max(0, pendingInboxNames.length - PENDING_INBOX_PREVIEW_LIMIT),
  ...
  const server = http.createServer(async (req, res) => {
    if ((req.method === 'GET' || req.method === 'HEAD') && req.url === '/health') {
      ...
      const body = {
        ...
        start_command: 'node scripts/nectar-dispatch-bridge.js',
        next_inbox_command: NEXT_INBOX_COMMAND,
        next_inbox_summary_command: firstPendingInboxPath ? NEXT_INBOX_SUMMARY_COMMAND : null,
        next_inbox_path_command: firstPendingInboxPath ? NEXT_INBOX_PATH_COMMAND : null,
        pending_inbox_review_command: firstPendingInboxPath ? NEXT_INBOX_COMMAND : null,
        pending_inbox_path_command: firstPendingInboxPath ? NEXT_INBOX_PATH_COMMAND : null,
        pending_inbox_prompt_command: firstPendingInboxPath ? NEXT_INBOX_PROMPT_COMMAND : null,
