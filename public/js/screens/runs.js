...
function workerBadge(workerType) {
  ...
  return `<span style="font-size:10px;font-weight:600;letter-spacing:.04em;padding:2px 7px;border-radius:999px;background:${color}18;color:${color};border:1px solid ${color}40">${escapeHtml(label)}</span>`;
}
    ...
    el.innerHTML = `
      ...
      <div class="card" style="margin-bottom:var(--gap);display:flex;gap:12px;align-items:center;flex-wrap:wrap;padding:12px 16px">
        ...
        <select id="filter-worker" style="font-size:12px;padding:4px 8px;border-radius:6px;border:1px solid var(--border);background:var(--bg-surface);color:var(--text-primary);cursor:pointer">
          ...
          ${Object.keys(WORKER_COLORS).map(w => `<option value="${escapeAttr(w)}" ${activeFilter.worker_type === w ? 'selected' : ''}>${escapeHtml(w.replace('-worker',''))}</option>`).join('')}
        </select>
        <select id="filter-status" style="font-size:12px;padding:4px 8px;border-radius:6px;border:1px solid var(--border);background:var(--bg-surface);color:var(--text-primary);cursor:pointer">
          ...
          ${['running','completed','failed','pending'].map(s => `<option value="${escapeAttr(s)}" ${activeFilter.status === s ? 'selected' : ''}>${escapeHtml(s)}</option>`).join('')}
        </select>
...
function runRow(r) {
  ...
  return `<tr data-run-id="${escapeAttr(r.id)}">
    ...
    <td style="font-family:var(--font-instrument);font-size:12px">${escapeHtml(duration)}</td>
    <td><button class="btn btn-ghost btn-sm run-expand" data-id="${escapeAttr(r.id)}">Detail</button></td>
  </tr>`;
}
  ...
  detailEl.innerHTML = `
    <div class="card">
      <div class="card-header">
        <span class="card-title">Run Detail — ${escapeHtml(run.agent_name)}${run.worker_type ? ` · ${workerBadge(run.worker_type)}` : ''}</span>
      ...
      ${steps.length > 0 ? `
        ...
        <div style="font-size:12px;font-family:var(--font-instrument);background:var(--bg-surface);border-radius:var(--radius-sm);padding:12px;max-height:200px;overflow-y:auto">
          ${steps.map(s => `<div style="padding:2px 0;border-bottom:1px solid var(--border)">${escapeHtml(JSON.stringify(s))}</div>`).join('')}
        </div>` : ''}
      ${logs.length > 0 ? `
        ...
        <div style="font-size:11px;font-family:monospace;background:var(--bg-base);border-radius:var(--radius-sm);padding:12px;max-height:160px;overflow-y:auto;color:var(--color-ash)">
          ${logs.map(l => `<div>${escapeHtml(typeof l === 'string' ? l : JSON.stringify(l))}</div>`).join('')}
        </div>` : ''}
