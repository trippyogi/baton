...
async function dispatchRun({ db, runId, intent = 'orchestrate', instructions = [] }) {
  ...
  const touch = run.touch_id ? db.prepare('SELECT * FROM flow_touches WHERE id = ?').get(run.touch_id) : null;
  ...
  if (!agent || !dispatch.enabled || dispatch.transport === 'manual') {
    db.prepare(`UPDATE flow_touches SET status = 'active', updated_at = datetime('now') WHERE id = ?`).run(run.touch_id);
  ...
  if (result.dispatch_status === 'not_configured') {
    ...
    if (run.touch_id) db.prepare(`UPDATE flow_touches SET status = 'active', updated_at = datetime('now') WHERE id = ?`).run(run.touch_id);
    rebuildTouches(db);
...
function applyAccepted(db, { runId, taskId, touchId, agentId, externalRunId }) {
  const tx = db.transaction(() => {
    ...
    if (touchId) db.prepare(`UPDATE flow_touches SET status = 'passed', run_id = ?, updated_at = datetime('now') WHERE id = ?`).run(runId, touchId);
...
function applyFailed(db, { runId, taskId, touchId, agentId, dispatchStatus, error }) {
  const tx = db.transaction(() => {
    ...
    if (touchId) db.prepare(`UPDATE flow_touches SET status = 'active', updated_at = datetime('now') WHERE id = ?`).run(touchId);
